...
export default function Home() {
  ...
  return <main>
    <header><div><p className="eyebrow">LOCAL HANDWRITING WORKSPACE</p><h1>Doooodle</h1></div><label className="upload">{busy ? "Processing..." : "Scan notes"}<input type="file" accept="image/*" capture="environment" onChange={upload} disabled={busy} /></label></header>
    ...
    <section className="canvas" onPointerDown={e => { if (e.target === e.currentTarget) pan.current = { x: e.clientX - view.x, y: e.clientY - view.y }; }} onPointerMove={e => { move(e); if (pan.current) setView(current => ({ ...current, x: e.clientX - pan.current!.x, y: e.clientY - pan.current!.y })); }} onPointerUp={() => { up(); pan.current = null; }} onWheel={e => { e.preventDefault(); setView(current => ({ ...current, scale: Math.min(2, Math.max(.4, current.scale + (e.deltaY < 0 ? .1 : -.1))) })); }}>
